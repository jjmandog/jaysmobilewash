/**
 * Moonlight 16B A3B Instruct API Handler (Premium)
 * Handles POST requests to /api/moonlight-16b for AI chat functionality using Moonshot's Moonlight 16B A3B Instruct model
 *
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const selectedModel = 'moonshotai/moonlight-16b-a3b-instruct';
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'X-Title': 'Jay\'s Mobile Wash Moonlight'
      },
      body: JSON.stringify({
        model: selectedModel,
    ...
    if (!response.ok) {
      ...
      res.end(JSON.stringify({ 
        error: 'Moonlight API request failed',
        details: errorText,
    ...
    if (data.choices && data.choices.length > 0) {
      ...
      res.end(JSON.stringify({
        responseText,
        selectedModel: selectedModel,
        source: 'openrouter-moonlight'
      }));
    } else {
      res.writeHead(500, corsHeaders);
      res.end(JSON.stringify({ 
        error: 'No response from Moonlight model',
        selectedModel: selectedModel
      }));
    }

  } catch (error) {
    console.error('Moonlight handler error:', error);
    res.writeHead(500, corsHeaders);
    res.end(JSON.stringify({ 
      error: 'Moonlight handler error: ' + error.message,
      selectedModel: 'moonshotai/moonlight-16b-a3b-instruct'
    }));
  }
}
