/**
 * Kimi VL A3B Thinking API Handler (Free)
 * Handles POST requests to /api/kimi-vl-a3b for AI chat functionality using Moonshot's Kimi VL A3B Thinking model
 *
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Kimi VL A3B Thinking model configuration
    const selectedModel = 'moonshotai/kimi-vl-a3b-thinking:free';
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'X-Title': 'Jay\'s Mobile Wash Kimi VL'
      },
      body: JSON.stringify({
        model: selectedModel,
    ...
    if (!response.ok) {
      ...
      res.end(JSON.stringify({ 
        error: 'Kimi VL API request failed',
        details: errorText,
    ...
    if (data.choices && data.choices.length > 0) {
      ...
      res.end(JSON.stringify({
        responseText,
        selectedModel: selectedModel,
        source: 'openrouter-kimi-vl'
      }));
    } else {
      res.writeHead(500, corsHeaders);
      res.end(JSON.stringify({ 
        error: 'No response from Kimi VL model',
        selectedModel: selectedModel
      }));
    }

  } catch (error) {
    console.error('Kimi VL handler error:', error);
    res.writeHead(500, corsHeaders);
    res.end(JSON.stringify({ 
      error: 'Kimi VL handler error: ' + error.message,
      selectedModel: 'moonshotai/kimi-vl-a3b-thinking:free'
    }));
  }
}
