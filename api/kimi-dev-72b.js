/**
 * Kimi Dev 72B API Handler (Free)
 * Handles POST requests to /api/kimi-dev-72b for AI chat functionality using Moonshot's Kimi Dev 72B model
 *
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const selectedModel = 'moonshotai/kimi-dev-72b:free';
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'X-Title': 'Jay\'s Mobile Wash Kimi Dev'
      },
      body: JSON.stringify({
        model: selectedModel,
    ...
    if (!response.ok) {
      ...
      res.end(JSON.stringify({ 
        error: 'Kimi Dev API request failed',
        details: errorText,
    ...
    if (data.choices && data.choices.length > 0) {
      ...
      res.end(JSON.stringify({
        responseText,
        selectedModel: selectedModel,
        source: 'openrouter-kimi-dev'
      }));
    } else {
      res.writeHead(500, corsHeaders);
      res.end(JSON.stringify({ 
        error: 'No response from Kimi Dev model',
        selectedModel: selectedModel
      }));
    }

  } catch (error) {
    console.error('Kimi Dev handler error:', error);
    res.writeHead(500, corsHeaders);
    res.end(JSON.stringify({ 
      error: 'Kimi Dev handler error: ' + error.message,
      selectedModel: 'moonshotai/kimi-dev-72b:free'
    }));
  }
}
