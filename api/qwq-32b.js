/**
 * QWQ 32B API Handler (Free)
 * Handles POST requests to /api/qwq-32b for AI chat functionality using ArliAI's QWQ 32B model
 *
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // QWQ 32B model configuration
    const selectedModel = 'arliai/qwq-32b-arliai-rpr-v1:free';
    ...
    if (data.choices && data.choices.length > 0) {
      ...
      res.end(JSON.stringify({
        responseText,
        selectedModel: selectedModel,
        source: 'openrouter-qwq'
      }));
    } else {
      res.writeHead(500, corsHeaders);
      res.end(JSON.stringify({ 
        error: 'No response from QWQ model',
        selectedModel: selectedModel
      }));
    }

  } catch (error) {
    console.error('QWQ handler error:', error);
    res.writeHead(500, corsHeaders);
    res.end(JSON.stringify({ 
      error: 'QWQ handler error: ' + error.message,
      selectedModel: 'arliai/qwq-32b-arliai-rpr-v1:free'
    }));
  }
}
