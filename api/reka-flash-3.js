/**
 * Reka Flash 3 API Handler (Free)
 * Handles POST requests to /api/reka-flash-3 for AI chat functionality using Reka AI's Flash 3 model
 *
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Reka Flash 3 model configuration
    const selectedModel = 'rekaai/reka-flash-3:free';
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'X-Title': 'Jay\'s Mobile Wash Reka Flash'
      },
      body: JSON.stringify({
        model: selectedModel,
    ...
    if (!response.ok) {
      ...
      res.end(JSON.stringify({ 
        error: 'Reka Flash API request failed',
        details: errorText,
    ...
    if (data.choices && data.choices.length > 0) {
      ...
      res.end(JSON.stringify({
        responseText,
        selectedModel: selectedModel,
        source: 'openrouter-reka-flash'
      }));
    } else {
      res.writeHead(500, corsHeaders);
      res.end(JSON.stringify({ 
        error: 'No response from Reka Flash model',
        selectedModel: selectedModel
      }));
    }

  } catch (error) {
    console.error('Reka Flash handler error:', error);
    res.writeHead(500, corsHeaders);
    res.end(JSON.stringify({ 
      error: 'Reka Flash handler error: ' + error.message,
      selectedModel: 'rekaai/reka-flash-3:free'
    }));
  }
}
