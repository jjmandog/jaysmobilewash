/**
 * Enhanced Auto Mode API Handler
 ...
 * - Support for all available models (OpenRouter, DeepSeek, specialized models)
 * - File upload detection with automatic vision model routing
 * - Robust fallback system with multiple backup options
 * - Detailed confidence scoring and reasoning
 * 
 * Supported Models:
 * - Qwen 3 235B: Enterprise & sophisticated reasoning
 * - QWQ 32B: Deep analytical reasoning & step-by-step thinking
 * - GLM-Z1 32B: Technical analysis & complex systems
 * - Kimi Dev 72B: Programming & development tasks
 * - Kimi VL A3B: Visual analysis & multimodal reasoning
 * - Qwerky 72B: Creative & writing tasks
 * - Llama 3.2 Vision: Secondary vision model
 * - Nemotron Super 49B: Performance-critical tasks
 * - Llama 4 Maverick: Advanced reasoning & problem-solving
 * - Llama 4 Scout: General purpose & balanced tasks
 * - Moonlight 16B: Business & service queries
 * - Dolphin Mistral 24B: Assisted tasks & tutorials
 * - Reka Flash 3: Quick & simple queries
 * - DeepSeek: Reliable fallback for general queries
 * 
 * Expected request body: { prompt: string, role?: string, model?: string, messages?: array }
 * Returns: { responseText: string, selectedModel: string, selectedHandler: string, autoMode: object }
 */
...
function analyzePromptForBestHandler(prompt, messages) {
  ...
  // File upload detection - prioritize best vision models
  if (messages && messages.some(msg => msg.type === 'image' || msg.attachments)) {
    return {
      handler: '/api/kimi-vl-a3b',
      model: 'kimi_vl_a3b',
      reason: 'Image/file upload detected - using Kimi VL A3B',
      confidence: 0.96
    };
  }

  // === ENTERPRISE & ADVANCED REASONING ===
  // Qwen 3 235B - Best for sophisticated enterprise analysis and large-scale reasoning
  if (msgLower.includes('enterprise') || msgLower.includes('business strategy') || msgLower.includes('sophisticated') ||
      ...
      msgLower.includes('comprehensive analysis') || msgLower.includes('detailed breakdown') || msgLower.length > 300) {
    return {
  ...
  // QWQ 32B - Best for complex analytical tasks and step-by-step reasoning
  if (msgLower.includes('analyze') || msgLower.includes('reasoning') || msgLower.includes('logic') ||
      ...
      msgLower.includes('conclude') || msgLower.includes('critical thinking') || msgLower.includes('step by step') ||
      msgLower.includes('chain of thought') || msgLower.includes('reasoning process') || 
      (msgLower.length > 200 && msgLower.length <= 300)) {
    return {
      handler: '/api/qwq-32b',
      model: 'qwq_32b',
  ...
  if (msgLower.includes('technical') || msgLower.includes('explain') || msgLower.includes('how does') ||
      ...
      msgLower.includes('methodology') || msgLower.includes('process') || msgLower.includes('framework')) {
    return {
  ...
  // === VISUAL & MULTIMODAL ===
  // Kimi VL A3B - Specialized for visual analysis and multimodal reasoning
  if (msgLower.includes('image') || msgLower.includes('visual') || msgLower.includes('photo') ||
      ...
      msgLower.includes('multimodal') || msgLower.includes('visual reasoning')) {
    return {
      handler: '/api/kimi-vl-a3b',
      model: 'kimi_vl_a3b',
      reason: 'Visual/multimodal analysis - using Kimi VL A3B',
      confidence: 0.92
    };
  }

  // === LLAMA 3.2 VISION FALLBACK ===
  // Llama 3.2 Vision - Secondary option for visual tasks
  if (msgLower.includes('vision') || msgLower.includes('computer vision') || msgLower.includes('image recognition')) {
    return {
      handler: '/api/llama32-vision',
      model: 'llama32_vision',
      reason: 'Vision tasks - using Llama 3.2 Vision',
      confidence: 0.85
    };
  }
  ...
  // Reka Flash 3 - Best for quick, simple questions and fast responses
  if (msgLower.length < 50 || msgLower.includes('hello') || msgLower.includes('hi ') || 
      ...
      msgLower.includes('simple') || msgLower.includes('briefly') || msgLower.includes('fast') ||
      msgLower.includes('short answer') || msgLower.includes('one word') || msgLower.includes('yes or no')) {
    return {
  ...
  // === DEEPSEEK FALLBACK ===
  // DeepSeek - Reliable fallback for general queries
  if (msgLower.includes('deepseek') || msgLower.includes('general') || msgLower.includes('balanced')) {
    return {
      handler: '/api/deepseek',
      model: 'deepseek',
      reason: 'General/balanced query - using DeepSeek',
      confidence: 0.75
    };
  }
...
async function routeToHandler(analysis, originalBody) {
  try {
    ...
    try {
      ...
      const fallbackResponse = await fetch('https://jaysmobilewash.net/api/deepseek', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (fallbackResponse.ok) {
        ...
        fallbackData.fallbackHandler = '/api/deepseek';
        fallbackData.fallbackModel = 'deepseek';
        return fallbackData;
      }
    } catch (fallbackError) {
      console.error('❌ DeepSeek fallback failed:', fallbackError);
      
      // Try Llama 4 Scout as final fallback
      try {
        console.log('🔄 Final fallback to Llama 4 Scout...');
        
        const finalFallbackResponse = await fetch('https://jaysmobilewash.net/api/llama4-scout', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (finalFallbackResponse.ok) {
          ...
          finalFallbackData.fallback = true;
          finalFallbackData.originalError = error.message;
          finalFallbackData.fallbackHandler = '/api/llama4-scout';
          finalFallbackData.fallbackModel = 'llama4_scout';
