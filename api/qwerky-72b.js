/**
 * Qwerky 72B API Handler (Free)
 * Handles POST requests to /api/qwerky-72b for AI chat functionality using Featherless's Qwerky 72B model
 *
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Qwerky 72B model configuration
    const selectedModel = 'featherless/qwerky-72b:free';
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'X-Title': 'Jay\'s Mobile Wash Qwerky'
      },
      body: JSON.stringify({
        model: selectedModel,
    ...
    if (!response.ok) {
      ...
      res.end(JSON.stringify({ 
        error: 'Qwerky API request failed',
        details: errorText,
    ...
    if (data.choices && data.choices.length > 0) {
      ...
      res.end(JSON.stringify({
        responseText,
        selectedModel: selectedModel,
        source: 'openrouter-qwerky'
      }));
    } else {
      res.writeHead(500, corsHeaders);
      res.end(JSON.stringify({ 
        error: 'No response from Qwerky model',
        selectedModel: selectedModel
      }));
    }

  } catch (error) {
    console.error('Qwerky handler error:', error);
    res.writeHead(500, corsHeaders);
    res.end(JSON.stringify({ 
      error: 'Qwerky handler error: ' + error.message,
      selectedModel: 'featherless/qwerky-72b:free'
    }));
  }
}
