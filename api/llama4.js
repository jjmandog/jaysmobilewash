/**
 * Llama 4 API Handler (Gated Access)
 * Handles POST requests to /api/llama4 for AI chat functionality using Meta's Llama 4 models
 *
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Map specific Llama 4 API IDs to OpenRouter model names
    const modelMapping = {
    ...
    // Use OpenRouter API for reliability
    ...
    const systemPrompt = `You are Jay's Mobile Wash AI assistant with advanced reasoning capabilities. Always answer in a friendly, human tone. Use Jay's business info ONLY if the user asks about services, pricing, location, or contact. For other topics, answer as a general AI assistant.
...
Website: jaysmobilewash.net`;
    ...
    console.log('🦙 Using Llama 4 model (via OpenRouter):', selectedModel);
    ...
    if (!response.ok) {
      ...
      res.end(JSON.stringify({
        error: `Llama 4 API Error: ${response.status} - ${errorText}`,
        model: selectedModel
      }));
      return;
    }
    ...
    console.error('❌ Llama 4 API Error:', error);
    res.writeHead(500, corsHeaders);
