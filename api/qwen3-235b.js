/**
 * Qwen 3 235B A22B API Handler (Free)
 * Handles POST requests to /api/qwen3-235b for AI chat functionality using Alibaba's Qwen 3 235B A22B model
 *
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Qwen 3 235B A22B model configuration
    const selectedModel = 'qwen/qwen3-235b-a22b:free';
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'X-Title': 'Jay\'s Mobile Wash Qwen 3'
      },
      body: JSON.stringify({
        model: selectedModel,
    ...
    if (!response.ok) {
      ...
      res.end(JSON.stringify({ 
        error: 'Qwen 3 API request failed',
        details: errorText,
    ...
    if (data.choices && data.choices.length > 0) {
      ...
      res.end(JSON.stringify({
        responseText,
        selectedModel: selectedModel,
        source: 'openrouter-qwen3'
      }));
    } else {
      res.writeHead(500, corsHeaders);
      res.end(JSON.stringify({ 
        error: 'No response from Qwen 3 model',
        selectedModel: selectedModel
      }));
    }

  } catch (error) {
    console.error('Qwen 3 handler error:', error);
    res.writeHead(500, corsHeaders);
    res.end(JSON.stringify({ 
      error: 'Qwen 3 handler error: ' + error.message,
