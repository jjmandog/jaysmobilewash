/**
 * Llama 3.2 11B Vision Instruct API Handler (Free)
 * Handles POST requests to /api/llama32-vision for AI chat functionality using Meta's Llama 3.2 11B Vision Instruct model
 *
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Llama 3.2 11B Vision Instruct model configuration
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'X-Title': 'Jay\'s Mobile Wash Llama Vision'
      },
      body: JSON.stringify({
        model: selectedModel,
    ...
    if (!response.ok) {
      ...
      res.end(JSON.stringify({ 
        error: 'Llama Vision API request failed',
        details: errorText,
    ...
    if (data.choices && data.choices.length > 0) {
      ...
      res.end(JSON.stringify({
        responseText,
        selectedModel: selectedModel,
        source: 'openrouter-llama-vision'
      }));
    } else {
      res.writeHead(500, corsHeaders);
      res.end(JSON.stringify({ 
        error: 'No response from Llama Vision model',
        selectedModel: selectedModel
      }));
    }

  } catch (error) {
    console.error('Llama Vision handler error:', error);
    res.writeHead(500, corsHeaders);
    res.end(JSON.stringify({ 
      error: 'Llama Vision handler error: ' + error.message,
