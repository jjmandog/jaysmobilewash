/**
 * Llama 4 Scout API Handler (Free)
 * Handles POST requests to /api/llama4-scout for AI chat functionality using Meta's Llama 4 Scout model
 *
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const selectedModel = 'meta-llama/llama-4-scout:free';
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'X-Title': 'Jay\'s Mobile Wash Llama 4 Scout'
      },
      body: JSON.stringify({
        model: selectedModel,
    ...
    if (!response.ok) {
      ...
      res.end(JSON.stringify({ 
        error: 'Llama 4 Scout API request failed',
        details: errorText,
    ...
    if (data.choices && data.choices.length > 0) {
      ...
      res.end(JSON.stringify({
        responseText,
        selectedModel: selectedModel,
        source: 'openrouter-llama4-scout'
      }));
    } else {
      res.writeHead(500, corsHeaders);
      res.end(JSON.stringify({ 
        error: 'No response from Llama 4 Scout model',
        selectedModel: selectedModel
      }));
    }

  } catch (error) {
    console.error('Llama 4 Scout handler error:', error);
    res.writeHead(500, corsHeaders);
    res.end(JSON.stringify({ 
      error: 'Llama 4 Scout handler error: ' + error.message,
      selectedModel: 'meta-llama/llama-4-scout:free'
    }));
  }
}
